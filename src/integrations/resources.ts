...
class Resources {
  ...
  public getTimingObj() {
    ...
    let loadPageInfo: loadPageInterface = {
      ...
      loadPageTime:
        performanceTiming.loadEventEnd - performanceTiming.navigationStart || 0,
      //【重要】解析 DOM 树结构的时间
      //【原因】反省下你的 DOM 树嵌套是不是太多了！
      domReady:
        performanceTiming.domComplete - performanceTiming.responseEnd || 0,
      //【重要】重定向的时间
      //【原因】拒绝重定向！比如，http://example.com/ 就不该写成 http://example.com
      redirect:
        performanceTiming.redirectEnd - performanceTiming.redirectStart || 0,
      //【重要】DNS 查询时间
      ...
      request:
        performanceTiming.responseEnd - performanceTiming.requestStart || 0,
      //【重要】执行 onload 回调函数的时间
      //【原因】是否太多不必要的操作都放到 onload 回调函数里执行了，考虑过延迟加载、按需加载的策略么？
      loadEvent:
        performanceTiming.loadEventEnd - performanceTiming.loadEventStart || 0,
      // DNS 缓存时间
      appcache:
        performanceTiming.domainLookupStart - performanceTiming.fetchStart || 0,
      // 卸载页面的时间
      unloadEvent:
      ...
      // TCP 建立连接完成握手的时间
      connect:
        performanceTiming.connectEnd - performanceTiming.connectStart || 0
    };
  ...
  public listenerUrl() {
    ...
    setInterval(function() {
      ...
      let defaultLocation = sessionStorage.getItem("defaultLocation");
